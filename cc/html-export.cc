...
void add_nodes(fmt::memory_buffer& html, const acmacs::tal::v3::Node& node, const acmacs::tal::v3::Node& /*parent*/, double edge_scale, prefix_t& prefix, bool last)
{
    if (node.is_leaf()) {
        ...
        // if (node.number_leaves_in_subtree() >= 20) {
            ...
            //     fmt::format_to(html, "<td class='common-aa'>leaves:{} {}</td>", node.number_leaves_in_subtree(), rep);
        if (const auto rep = node.aa_transitions_.display(); !rep.empty())
            fmt::format_to(html, "<td class='common-aa'>{}leaves:{} {} -- left:{}</td>", node.seq_id.empty() ? std::string{} : fmt::format("[{}] ", node.seq_id), node.number_leaves_in_subtree(), rep, node.node_for_left_aa_transitions_ ? node.node_for_left_aa_transitions_->seq_id : std::string_view{});
        // }
