    ...
    if (auto hz_sections = tal().draw().layout().find<HzSections>(); hz_sections) {
        for (const auto& clade : clades_) {
            for (auto [section_no, clade_section] : acmacs::enumerate(clade.sections)) {
                hz_sections->add_section(HzSections::Section{HzSections::section_id_t{fmt::format("{}-{}", clade.name, section_no)}, clade_section.first, clade_section.last, true,
                                                             clade_section.display_name, AA_Transitions{}});
            }
        }
    }
